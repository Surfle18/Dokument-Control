    ...
    if ($stmt_select) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $stmt_update = $conn->prepare("UPDATE balance_handover SET app2 = ?, supervisor = ? WHERE id = ?");
                ...
                if ($stmt_update) {
                    ...
                    if ($stmt_update->execute()) {
                        ...
                        header("Location: http://10.11.11.176/dokumentcontrol/approvement/balance/views.php?id=$id&BERHASIL✅");
                        exit();
                    } else {
                        session_start();
                        session_unset();
